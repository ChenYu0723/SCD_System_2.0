...
# 所有station训练一个model，每次epoch都随机测试5000个样本的mape，记录所有mape并画图
...
def main():
    ...
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig(path + r'/result/mape_all.png', dpi=150)

    plt.plot(range(1, EPOCH+1), acc_all_ls, c='r', label='Training Accuracy')  # ls='--', marker='^',
    plt.plot(range(1, EPOCH+1), acc_test_all_ls, c='b', label='Testing Accuracy')  # ls='--', marker='^',
    # plt.xticks(range(1, EPOCH+1))
    # plt.yticks(range(0, 110, 10))
    # plt.ylim((0, 100))
    plt.xlabel('Epoch')
    ...
    plt.legend(loc='best')
    plt.tight_layout()
    plt.savefig(path + r'/result/acc_all.png', dpi=150)
    ...
    torch.save(lstm, 'net_lstm_3.pkl')
