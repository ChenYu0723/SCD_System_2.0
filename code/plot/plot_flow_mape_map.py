...
for i in range(len(sta_mape_df)):
    ...
    folium.CircleMarker(location=[lat, lon], radius=get_logsize(sta_mape_df.iloc[i, 3]), popup='mape', color='#4169E1', fill=True,
                        fill_color='#EE1289', ).add_to(m)

# ==== plot all mape
fig = plt.figure()
plt.plot(sta_mape_df['mape'])
