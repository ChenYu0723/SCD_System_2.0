...
def main(STATION_ID):
    ...
    lstm = torch.load(path + r'/model/single_station_model/net_lstm_%s.pkl' % STATION_ID)
    ...
    plt.plot(range(len(test_y)), test_y, ls='-', label='真实值')
    plt.plot(range(len(test_y)), pred_outs, ls='--', label='预测值')
    # plt.text(0, 0, 'Loss=%.5f' % loss.item(), fontdict={'size': 10, 'color': 'red'})
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.xlabel('时隙/15min', fontsize=16)
    plt.ylabel('人流量/人次', fontsize=16)
    # plt.title('Predict result of %s' % STATION_ID)
    plt.legend(loc='best', fontsize=14)
    plt.tight_layout()
    plt.savefig(path + r'/result/station_flow_prediction_%s.png' % STATION_ID, dpi=300)
...
def test():
    ...
    raw_flow = pd.read_csv(infile)
    # print(raw_flow)
    station_flow = raw_flow[raw_flow[' station'] == 1722]
    print(station_flow)
...
if __name__ == '__main__':
    ...
    STATION_ID = 2040  # 113 247 2010 2011 2035 2040
    main(STATION_ID)
    # main_2()
    # test()
